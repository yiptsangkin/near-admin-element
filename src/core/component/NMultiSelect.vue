<template>
    <div>
        ...
        >
            <el-row class="multi-title" type="flex">
                <el-col key="multi-sort" style="width: 55px" v-if="showSortIndex">序号</el-col>
                <el-col v-for="(item, index) in optionColumns" :key="index" :style="computedWidth(item.width)">{{ item.title }}{{ showTitleField ? `(${item.field})` : ''}}</el-col>
            </el-row>
            <el-option v-for="(item, index) in targetOption" :label="item[labelProp]" :key="index" :value="item">
                <el-row class="multi-option" type="flex">
                    <el-col class="multi-option-index" style="width: 50px" v-if="showSortIndex">
                        {{ pagingInfo ? ((pagingInfo.currentPage - 1) * pagingInfo.pageSize + index + 1) : index + 1 }}</el-col>
                    <el-col v-for="(citem, cindex) in optionColumns" :key="cindex" :style="computedWidth(citem.width)">
                        {{ item[citem.field] }}
                    </el-col>
                </el-row>
            </el-option>
